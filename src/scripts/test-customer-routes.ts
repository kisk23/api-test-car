...
async function test() {
  try {
    ...
    const dealerLoginRes = await fetch(`${BASE_URL}/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: dealerData.email, password: dealerData.password })
    });
    ...
    if (bookingId) {
        ...
        const bookingByIdRes = await fetch(`${BASE_URL}/customer/bookings/${bookingId}`, {
            headers: { 'Authorization': `Bearer ${customerToken}` }
        });
        ...
        const cancelRes = await fetch(`${BASE_URL}/customer/bookings/${bookingId}/cancel`, {
            method: 'PATCH',
            headers: { 'Authorization': `Bearer ${customerToken}` }
        });
